...
def get_map(MINOVERLAP, draw_plot, path='./map_out', hosptal=""):
    ...
    # for item in open("6000").read().strip().split():
    #     ground_truth_files_list.append(GT_PATH + '/'+item+'.txt')
    ...
    if draw_plot:
        window_title = "mAP"
        plot_title = "mAP=%0.3f" % mAP
        x_label = "AP"
        output_path = path + "/%0.1f_%s_mAP.pdf" % (MINOVERLAP, hosptal)
        to_show = True
        plot_color = 'royalblue'
        draw_plot_func(
            ap_dictionary,
