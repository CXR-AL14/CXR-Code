...
if __name__ == '__main__':
    ...
    state_dictunet = torch.load('weight-model-AttUnet-Lung_Herat-1-512-XZHData-0.95983.pth', map_location=device)
    CTR.load_state_dict(state_dictunet)
    # CTR = torch.load("718.pth")
    CTR.to(device)
    i = 0

    columns = ["img_path", "lung_x_begin", "lung_x_end", "lung_y", "center_x", "heart_x_begin_left",
               "heart_y_begin_left",
               "heart_x_end_right", "heart_y_end_right", "ratio", "time"]

    # img_lists = os.listdir("Test_Image")
    # img_lists.sort()


    # for name in ["2000","fd","nc","xs","qzx"]:
    for name in ["bn","fd","nc","xs","qzx",]:
        data = []
        save_image_dir = "/home/Yang/Project/CRT/images_%s" % name
        if not os.path.exists(save_image_dir):
            os.makedirs(save_image_dir)
        for root, dirs, files in os.walk("/home/Yang/Project/CRT/%s" % name):
            files.sort()
            for im_name in tqdm(files):
                # 计时开始：
                start = time.time()
                # img_path = "Test_Image/" + im_name
                ...
                contours, hierachy = cv2.findContours(lung, cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE)
                contours2, hierachy2 = cv2.findContours(heart, cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE)
                result = cv2.drawContours(img, contours, -1, (0, 0, 255), 20)
                ...
                if ctr != None:
                    ...
                    cv2.line(result, (int(ctr["heart_x_begin_left"]), int(ctr["heart_y_begin_left"])),
                             (int((ctr["lung_x_end"] + ctr["lung_x_begin"]) / 2), int(ctr["heart_y_begin_left"])),
                             (0, 0, 0),
                             20)
                    ...
                    cv2.putText(result, "CTR:%.4f" % round(ctr["ratio"], 4), (100, 200), font, 5, (0, 255, 0), 10)

                result = Image.fromarray(cv2.cvtColor(result, cv2.COLOR_BGR2RGB))
                # result.show()

                result.save(os.path.join(save_image_dir, im_name))

                try:
                    lung_x_begin = ctr["lung_x_begin"]
                    lung_x_end = ctr["lung_x_end"]
                    lung_y = ctr["lung_y"]
                    center_x = ctr["center_x"]
                    heart_x_begin_left = ctr["heart_x_begin_left"]
                    heart_y_begin_left = ctr["heart_y_begin_left"]
                    heart_x_end_right = ctr["heart_x_end_right"]
                    heart_y_end_right = ctr["heart_y_end_right"]
                    ratio = ctr["ratio"]
                except:
                    lung_x_begin = None
                    lung_x_end = None
                    lung_y = None
                    center_x = None
                    heart_x_begin_left = None
                    heart_y_begin_left = None
                    heart_x_end_right = None
                    heart_y_end_right = None
                    ratio = None
                end = time.time()
                data.append(
                    [im_name, lung_x_begin, lung_x_end, lung_y, center_x, heart_x_begin_left, heart_y_begin_left,
                     heart_x_end_right, heart_y_end_right, ratio, round(end - start, 4)])

        df = pd.DataFrame(data=data, columns=columns)
        df.to_csv("%s_718.csv" % name, index=False, sep=",", encoding="utf-8")
